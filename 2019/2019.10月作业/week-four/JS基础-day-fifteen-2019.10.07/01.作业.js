...
// 70.计算下列表达式的结果：，并写出详细步骤
 var a = 10;
 var b = 2;
 a %=b;     // a=a%b a=10%2    10%2没有余数   所以  a=0
console.log(a); //  0
console.log(b++);//这里是后++  所以是先打印在赋值  所以打印的结果是2  打印完之后b变成了3



// 二、简答题
// 1. console.log(0.1+.03)结果是什么，为什么  
//结果为 0.13    算式.03自动补全  就等于console.log(0.1+0.03)   所以结果是0.13
// 2. 怎么将一个小数，取小数点的后2位
// (0.123456789).toFixed(2)
// 3. 什么是隐式类型转换，什么时候发生
// 当两个不同类型的数据进行计算时,JS会自动转换为同样的类型再计算
// 4. 类型转换时，Number和parseInt的区别是什么
// Number转换如果有非数字字符就会转换成NaN
// parseInt转换时如果有非数字字符就只会输出数字